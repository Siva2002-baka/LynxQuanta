...
class LynxBrain(Node):
    ...
    SPIN_WHEEL_MIN = 0.00
    SPIN_WHEEL_MAX = 0.22
    SPIN_WHEEL_YAW_GAIN = 0.50
    SPIN_WHEEL_RAMP = 0.020

    DEFAULT_SPIN_ANG_Z = 0.30
    ...
    def __init__(self):
        ...
        # ------------------------------------------------------------------
        # Navigation-specific /cmd_vel handling
        # ------------------------------------------------------------------
        # Nav2/MPPI uses ROS convention: +linear.x is forward, +angular.z is CCW.
        # Your Gazebo/robot wheel convention is opposite for forward wheel drive:
        # teleop W currently sends negative linear.x for forward.
        # Therefore navigation wheel drive intentionally inverts linear.x.
        self.NAV_DEADMAN_TIMEOUT = 0.35
        self.NAV_LIN_DEADBAND = 0.015
        self.NAV_YAW_DEADBAND = 0.015
        self.NAV_SPIN_YAW_START = 0.22
        self.NAV_SPIN_YAW_STRONG = 0.38
        self.NAV_SPIN_LIN_ALLOW = 0.10
        self.NAV_MAX_LIN = 0.24
        self.NAV_MAX_YAW = 0.60
        self.NAV_MIN_DRIVE_SCALE = 0.35
        ...
        self.get_logger().info(
            "NAVIGATION BRAIN V1 loaded: Nav2/MPPI cmd_vel bridge + original spin gait"
        )
    ...
    def cmd_vel_callback(self, msg):
        """
        Navigation-first cmd_vel bridge.

        Input convention from Nav2/MPPI:
          +linear.x  = forward
          +angular.z = CCW

        Output convention for this M20 Gazebo/controller setup:
          forward wheel drive needs negative wheel rate compared with ROS +x.

        Behavior:
          1) zero cmd_vel -> hard stop
          2) strong yaw demand -> original A/D leg+wheels spin
          3) small/moderate yaw while moving -> wheel drive forward/backward,
             with speed reduced as yaw demand increases

        This keeps the original spin gait. It only changes how mixed Nav2
        commands are interpreted, so MPPI no longer gets ignored when it sends
        linear.x and angular.z together.
        """
        ...
        lin = clamp(float(msg.linear.x), -self.NAV_MAX_LIN, self.NAV_MAX_LIN)
        yaw = clamp(float(msg.angular.z), -self.NAV_MAX_YAW, self.NAV_MAX_YAW)
        ...
        # STOP
        # --------------------------------------------------------------
        if lin_abs <= self.NAV_LIN_DEADBAND and yaw_abs <= self.NAV_YAW_DEADBAND:
            self.transition_active = False
            self.walking = False
            self.walk_type = "NONE"
        ...
        # --------------------------------------------------------------
        # ROTATE-IN-PLACE FOR LARGE HEADING ERROR
        # --------------------------------------------------------------
        # MPPI normally sends mixed linear+yaw. The old brain ignored those.
        # Here, if yaw is strong enough, use your original leg+wheels spin and
        # intentionally suppress forward motion. This is safer for the M20.
        if yaw_abs >= self.NAV_SPIN_YAW_START and (
            lin_abs <= self.NAV_SPIN_LIN_ALLOW or yaw_abs >= self.NAV_SPIN_YAW_STRONG
        ):
            ...
            self.spin_ang_z = clamp(yaw_abs, 0.10, 0.80)

            if now - self.last_nav_log_time > 0.50:
                self.get_logger().info(
                    f"NAV SPIN: lin={lin:.3f}, yaw={yaw:.3f} -> {self.walk_type}"
                )
                self.last_nav_log_time = now
            return

        # --------------------------------------------------------------
        # FORWARD/BACKWARD WHEEL DRIVE FOR SMALL/MODERATE YAW
        # --------------------------------------------------------------
        if lin_abs > self.NAV_LIN_DEADBAND:
            self.transition_active = False
            self.walking = False
            self.walk_type = "NONE"
            self.posture = "STAND"

            # Slow down as yaw request increases. We are not doing true arc
            # turning here; the brain uses rotate-drive-rotate behavior.
            yaw_ratio = clamp(yaw_abs / self.NAV_SPIN_YAW_START, 0.0, 1.0)
            drive_scale = 1.0 - (1.0 - self.NAV_MIN_DRIVE_SCALE) * yaw_ratio

            # IMPORTANT: invert ROS +linear.x into your wheel convention.
            wheel_rate = (-lin * drive_scale) / self.WHEEL_RADIUS

            if now - self.last_nav_log_time > 0.50:
                self.get_logger().info(
                    f"NAV WHEEL: lin={lin:.3f}, yaw={yaw:.3f}, "
                    f"scale={drive_scale:.2f}, wheel_rate={wheel_rate:.3f}"
                )
                self.last_nav_log_time = now
        ...
        # PURE YAW FALLBACK
        # --------------------------------------------------------------
        if yaw_abs > self.NAV_YAW_DEADBAND:
            new_type = "S_CCW" if yaw > 0.0 else "S_CW"
            ...
            if now - self.last_nav_log_time > 0.50:
                self.get_logger().info(
                    f"NAV PURE SPIN: yaw={yaw:.3f} -> {self.walk_type}"
                )
                self.last_nav_log_time = now
            return
    ...
    def _spin_stepping_legs(self, spin_sign):
        """
        Pasted two-beat diagonal-pair spin.
    ...
    # ------------------------------------------------------------------
    def tick(self):
        # Deadman: if Nav2 stops publishing while this node is moving, stop wheels.
        # This prevents a stale last command from driving forever.
        if (
            self.last_cmd_vel_time > 0.0
            and time.monotonic() - self.last_cmd_vel_time > self.NAV_DEADMAN_TIMEOUT
            and (self.walking or any(abs(w) > 1e-4 for w in self.target_wheels))
        ):
            self.transition_active = False
            self.walking = False
            self.walk_type = "NONE"
        ...
        if self.walk_type in ("S_CW", "S_CCW"):
            ...
            lift_gate = smoothstep((active_lift - 0.04) / 0.55)
            crawl = base_rate * lift_gate * spin_sign

            w = self._wheel_forward_pattern(crawl)
